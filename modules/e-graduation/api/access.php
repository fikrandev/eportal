<?php
/**
 * E-Graduation teacher access API.
 */
...
function listAccess()
{
    try {
        ...
        $classes = accessClassesFromActiveStudents($yearId);
...
function saveAccess()
{
    ...
    try {
        ...
        }, accessClassesFromActiveStudents($yearId)));
        if (!isset($allowedClasses[$kelas])) {
            json_response(400, false, 'Kelas wali tidak valid. Pilih kelas dari Data Siswa E-Graduation yang aktif.');
        }

        $stmt = db()->prepare("
            INSERT INTO grad_teacher_access (user_id, access_role, kelas, status)
            VALUES (?, 'wali_kelas', ?, ?)
...
function deleteAccess()
{
    ...
    try {
        $stmt = db()->prepare("DELETE FROM grad_teacher_access WHERE user_id = ?");
        $stmt->execute([$userId]);
...
function accessClassesFromActiveStudents($yearId)
{
    $stmt = db()->prepare("
