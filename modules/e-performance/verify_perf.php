...
$ptk_id = isset($_GET['id']) ? (int) $_GET['id'] : 0;
...
if ($ptk_id > 0 && $periode_id > 0) {
    try {
        $stmt = $db->prepare("
            SELECT pt.nama, pt.niy, pt.jabatan, COALESCE(u.tupoksi, pt.jenis_ptk) as tupoksi,
                   pd.nama_periode, pd.tahun_ajaran, pd.is_released
            FROM perf_ptk pt
            LEFT JOIN users u ON pt.niy COLLATE utf8mb4_unicode_ci = u.username
            JOIN perf_periode pd ON pd.id = ?
            WHERE pt.id = ?
        ");
...
<head>
    ...
    <title>Verifikasi Penilaian Kinerja - <?php echo h($schoolName); ?></title>
    ...
    <main class="verify-wrap">
        <section class="verify-card">
            <?php if ($valid && $row): ?>
                ...
                <div class="verify-row"><span>Perihal</span><span>:</span><span class="value">Penilaian Kinerja Guru (Lampiran)</span></div>
                ...
                <div class="verify-block-title">Identitas Pegawai</div>
                <div class="verify-row strong"><span>Nama</span><span>:</span><span class="value"><?php echo h($row['nama']); ?></span></div>
                <div class="verify-row strong"><span>NIY/NUPTK</span><span>:</span><span class="value"><?php echo h($row['niy']); ?></span></div>
                <div class="verify-row strong"><span>Jabatan</span><span>:</span><span class="value"><?php echo h($row['jabatan'] ?: '-'); ?></span></div>
                <div class="verify-row strong"><span>Tupoksi</span><span>:</span><span class="value"><?php echo h($row['tupoksi']); ?></span></div>
