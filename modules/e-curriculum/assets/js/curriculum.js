...
const Curriculum = {
                ...
                <div class="sch-card-body">
                    <div style="margin-bottom:16px;display:flex;gap:12px;align-items:center;flex-wrap:wrap;justify-content:space-between;">
                        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                            ...
                            <button type="button" class="btn btn-sm" id="btnBukaRekapModal" onclick="Curriculum.showRekapDistribusiModal()" style="display:inline-flex;align-items:center;gap:6px;background:#f0fdf4;color:#166534;border:1px solid #bbf7d0;font-weight:600;padding:6px 13px;border-radius:8px;cursor:pointer;" title="Lihat Rekapitulasi Mengajar Semua Guru & Total JP">
                            ...
                            <div class="sch-view-switcher" style="display:inline-flex;background:#f1f5f9;border-radius:8px;padding:3px;border:1px solid #e2e8f0;">
                                <button type="button" class="btn btn-sm btn-dist-view active" id="btnDistViewTable" onclick="Curriculum.setDistViewMode('table')" style="border-radius:6px;font-size:0.8rem;padding:4px 10px;font-weight:600;border:none;background:#fff;color:#0f172a;box-shadow:0 1px 2px rgba(0,0,0,0.05);cursor:pointer;">
                                    📋 Per Penugasan
                                </button>
                                <button type="button" class="btn btn-sm btn-dist-view" id="btnDistViewRekap" onclick="Curriculum.setDistViewMode('rekap')" style="border-radius:6px;font-size:0.8rem;padding:4px 10px;font-weight:600;border:none;background:transparent;color:#64748b;cursor:pointer;">
                                    📊 Rekap Guru & JP
                                </button>
                            </div>
                        </div>
    ...
    showRekapDistribusiModal() {
        ...
        if (!rekapList.length) {
            EModal.toast({ type: 'warning', title: 'Perhatian', message: 'Belum ada data penugasan mengajar untuk direkap.' });
            return;
        }
            ...
            <div style="margin-bottom:12px;">
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:12px;margin-bottom:16px;">
                    <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px;padding:12px;text-align:center;">
                        <div style="font-size:11px;font-weight:600;color:#0369a1;text-transform:uppercase;">Total Guru Mengajar</div>
                    ...
                    <div style="background:#fef3c7;border:1px solid #fde68a;border-radius:10px;padding:12px;text-align:center;">
                        <div style="font-size:11px;font-weight:600;color:#92400e;text-transform:uppercase;">Total Jam Pelajaran</div>
                    ...
                    <div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:10px;padding:12px;text-align:center;">
                        <div style="font-size:11px;font-weight:600;color:#6d28d9;text-transform:uppercase;">Rata-rata Beban JP</div>
                        <div style="font-size:22px;font-weight:700;color:#7c3aed;margin-top:2px;">${rataRataJp} <span style="font-size:13px;font-weight:normal;">JP / Guru</span></div>
                    </div>
                    <div style="background:#ecfdf5;border:1px solid #a7f3d0;border-radius:10px;padding:12px;text-align:center;">
                        <div style="font-size:11px;font-weight:600;color:#047857;text-transform:uppercase;">Memenuhi &ge; 24 JP</div>
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:12px;">
                    <div style="position:relative;flex:1;max-width:320px;">
                        <input type="text" id="rekapModalSearchInput" class="form-input" placeholder="🔍 Cari guru, kelas, atau mapel..." oninput="Curriculum.filterRekapModalTable(this.value)" style="font-size:0.85rem;padding:7px 12px;">
                    </div>
                    <div style="display:flex;gap:8px;">
                        <button type="button" class="btn btn-outline btn-sm" onclick="Curriculum.printRekapDistribusi()" style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
                            Cetak Rekapitulasi
                        </button>
                        <button type="button" class="btn btn-outline btn-sm" onclick="Curriculum.exportRekapDistribusi()" style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;">
        ...
        EModal.form({
            title: 'Rekapitulasi Penugasan / Distribusi Mengajar Guru',
            size: 'xl',
            form: modalHtml,
            confirmText: 'Tutup',
            cancelText: 'Batal',
            onOpen: () => {
                $('.emodal-footer .btn-ghost').hide();
            },
            onConfirm: () => {
                EModal.closeAll();
                return true;
    ...
    exportRekapDistribusi() {
        const rekap = this.getRekapitulasiData('');
        if (!rekap.length) {
            EModal.toast({ type: 'warning', title: 'Perhatian', message: 'Tidak ada data untuk di-export.' });
            return;
        }

        this.loadSheetJS(() => {
            const rows = rekap.map((g, i) => {
                const rincian = g.assignments.map(a => `${a.nama_kelas} (${a.nama_mapel}: ${a.jp} JP)`).join(', ');
                const kelasList = Array.from(g.kelasSet).join(', ');
                const mapelList = Array.from(g.mapelSet).join(', ');
                return {
                    'No': i + 1,
                    'Nama Guru': g.nama_guru,
                    'NIP / Kode': g.kode_guru || '-',
                    'Mengajar di Mana Saja (Rincian)': rincian,
                    'Daftar Kelas': kelasList,
                    'Jumlah Kelas': g.kelasSet.size,
                    'Daftar Mapel': mapelList,
                    'Total JP': g.total_jp
                };
            });
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Rekap Mengajar Guru");
            XLSX.writeFile(wb, "Rekapitulasi_Distribusi_Mengajar_Guru.xlsx");
            EModal.toast({ type: 'success', title: 'Berhasil', message: 'File Excel rekapitulasi berhasil diunduh.' });
        });
    },
    ...
    printRekapDistribusi() {
        const rekap = this.getRekapitulasiData('');
        if (!rekap.length) {
            EModal.toast({ type: 'warning', title: 'Perhatian', message: 'Tidak ada data untuk dicetak.' });
            return;
        }
