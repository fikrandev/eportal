...
const Schedule = {
    ...
    renderDashboard($container) {
        $container.html(`
            <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(240px, 1fr));gap:24px;">
                <div class="sch-card" style="margin:0; border-top: 4px solid var(--primary)">
                    <div class="sch-card-body" style="text-align:center">
                        <svg width="48" height="48" stroke="var(--primary)" fill="none" stroke-width="1.5" style="margin-bottom:12px"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        <h2 style="margin:0;font-size:2rem;font-family:'Outfit'" id="dashGuru">-</h2>
                        <div style="color:var(--text-muted);font-size:0.85rem">Total Guru</div>
                    </div>
                </div>
                <div class="sch-card" style="margin:0; border-top: 4px solid var(--primary)">
                    <div class="sch-card-body" style="text-align:center">
                        <svg width="48" height="48" stroke="var(--primary)" fill="none" stroke-width="1.5" style="margin-bottom:12px"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg>
                        <h2 style="margin:0;font-size:2rem;font-family:'Outfit'" id="dashKelas">-</h2>
                        <div style="color:var(--text-muted);font-size:0.85rem">Total Kelas</div>
                    </div>
                </div>
                <div class="sch-card" style="margin:0; border-top: 4px solid var(--primary)">
                    <div class="sch-card-body" style="text-align:center">
                        <svg width="48" height="48" stroke="var(--primary)" fill="none" stroke-width="1.5" style="margin-bottom:12px"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                        <h2 style="margin:0;font-size:2rem;font-family:'Outfit'" id="dashKeb">-</h2>
                        <div style="color:var(--text-muted);font-size:0.85rem">Total Distribusi JP</div>
                    </div>
                </div>
            </div>
            
            <div class="sch-card" style="margin-top:24px">
                <div class="sch-card-header">
                    <h3>Aksi Cepat</h3>
                </div>
                <div class="sch-card-body" style="display:flex;gap:12px;flex-wrap:wrap">
                    <button class="btn btn-outline" onclick="Schedule.navigate('distribusi')">Atur Penugasan</button>
                    <button class="btn btn-outline" onclick="Schedule.navigate('kesediaan')">Matriks Kesediaan</button>
                    <button class="btn btn-primary" onclick="Schedule.navigate('jadwal')">Lihat & Generate Jadwal</button>
                </div>
            </div>
        `);
        ...
        $container.html(`
            <div class="sch-card">
                ...
                <div class="sch-card-body">
                    <div class="sch-table-wrapper" id="${type}Table"><div class="sch-empty">Memuat data...</div></div>
                </div>
            </div>
        `);
    ...
    loadMaster(type, meta) {
        this.api(type + '.php?action=list').done(res => {
            if (!res.data.length) {
                $('#' + type + 'Table').html('<div class="sch-empty">Data tidak ditemukan.</div>');
                return;
            }
            ...
            const rows = res.data.map(d => `
                <tr>
                    ...
                    <td style="width:120px">
                        <button class="btn btn-sm btn-outline" onclick="Schedule.formMaster('${type}', ${d.id}, '${meta.field1}','${meta.label1}','${meta.field2}','${meta.label2}', ${meta.isRombel}, '${d[meta.field1]}', '${d[meta.field2]}')">Edit</button>
                        <button class="btn btn-sm btn-danger" onclick="Schedule.deleteMaster('${type}', ${d.id})">Del</button>
                    </td>
                </tr>
            `).join('');
    ...
    renderJam($container) {
        $container.html(`
            <div class="sch-card">
                ...
                <div class="sch-card-body">
                    <div class="sch-table-wrapper" id="jamTable"><div class="sch-empty">Memuat...</div></div>
                </div>
            </div>
        `);
        ...
        this.api('jam.php?action=list').done(res => {
            ...
            if (!res.data.length) { $('#jamTable').html('<div class="sch-empty">Belum ada jam belajar.</div>'); return; }
            ...
            res.data.forEach((d, i) => {
                html += `<tr>
                    ...
                    <td>
                        <button class="btn btn-sm btn-outline" onclick="Schedule.formJam(${d.id})">Edit</button>
                        <button class="btn btn-sm btn-danger" onclick="Schedule.deleteMaster('jam', ${d.id})">Del</button>
                    </td>
                </tr>`;
            });
            html += '</tbody></table>';
        ...
        EModal.form({
            ...
            onConfirm: () => {
                ...
                this.api('jam.php?action=' + act, { method:'POST', data }).done(res => {
                    EModal.closeAll(); this.navigate('jam');
                });
                return false;
            }
        });
        ...
        $container.html(`
            <div class="sch-card">
                ...
                <div class="sch-card-body">
                    ...
                    <div class="sch-table-wrapper" id="distTable"><div class="sch-empty">Memuat...</div></div>
                </div>
            </div>
        `);
    ...
    loadDistribusi(kelasId = '') {
        this.api('distribusi.php?action=list').done(res => {
            ...
            if (!data.length) { $('#distTable').html('<div class="sch-empty">Belum ada distribusi.</div>'); return; }
            ...
            data.forEach((d, i) => {
                ...
                html += `<tr>
                    ...
                    <td>
                        <button class="btn btn-sm btn-outline" onclick="Schedule.formDist(${d.id})">Edit</button>
                        <button class="btn btn-sm btn-danger" onclick="Schedule.deleteMaster('distribusi', ${d.id})">Del</button>
                    </td>
                </tr>`;
            });
            ...
            EModal.form({
                ...
                onConfirm: () => {
                    ...
                    Promise.all(requests).then(() => {
                        EModal.closeAll(); this.navigate('distribusi');
                    });
                    return false;
                }
            });
    ...
    renderKesediaan($container) {
        $container.html(`
            <div class="sch-card">
                ...
                <div class="sch-card-body">
                    ...
                    <div class="matrix-container" id="kesediaanTable"><div class="sch-empty">Memuat matriks...</div></div>
        ...
        Promise.all([
            ...
            if (!data.length || !jams.length) { $('#kesediaanTable').html('<div class="sch-empty">Master Jam/Guru belum lengkap.</div>'); return; }
        ...
        $container.html(`
            <div class="sch-card">
                ...
                <div class="sch-card-body">
                    ...
                    <div id="jdwViewer"><div class="sch-empty">Memuat...</div></div>
                </div>
            </div>
        `);
    ...
    viewJadwal(kelasId) {
        ...
        Promise.all([
            ...
            if(!jadwal.length) { $('#jdwViewer').html('<div class="sch-empty">Jadwal masih kosong. Klik generate.</div>'); return; }
