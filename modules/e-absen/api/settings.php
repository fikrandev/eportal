...
function getWaGroupsProxy() {
    ...
    $groupsUrl = str_replace('/send', '/groups', $url);
    ...
    if ($response) {
        ...
        if ($resData && !empty($resData['success'])) {
            ...
            json_response(200, false, 'Gagal memuat grup', []);
        }
    } else {
        json_response(500, false, 'Server WA Node.js tidak merespon');
    }
}
...
function disconnectWaProxy() {
    ...
    $logoutUrl = str_replace('/send', '/logout', $url);
    ...
    if ($response) {
        $resData = json_decode($response, true);
        if ($resData && !empty($resData['success'])) {
            json_response(200, true, $resData['message'] ?? 'Koneksi WhatsApp berhasil diputuskan. Silakan scan QR Code baru.');
        } else {
            $msg = isset($resData['message']) ? $resData['message'] : 'Gagal memutuskan koneksi via Node.js';
            json_response(400, false, $msg);
        }
    } else {
        json_response(500, false, 'Server WA Node.js di port 3000 tidak merespon.');
    }
}
